...
def uniform_domain_randomize(
    ...
    def randomize_parameters(rng):
        ...
        frictionloss = jax.random.uniform(
            key, shape=(num_joints,), minval=0.0, maxval=1.0,
        )
        dof_frictionloss = mjx_model.dof_frictionloss.at[6:].set(frictionloss)
        ...
        damping = jax.random.uniform(
            key, shape=(num_joints,), minval=0.0, maxval=0.5,
        )
        dof_damping = mjx_model.dof_damping.at[6:].set(damping)
        ...
        armature = jax.random.uniform(
            key, shape=(num_joints,), minval=0.0, maxval=0.03,
        )
        dof_armature = mjx_model.dof_armature.at[6:].set(armature)
