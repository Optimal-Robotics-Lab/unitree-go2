...
class UnitreeGo2Env(mjx_env.MjxEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.total_mass = self._mj_model.body_mass.sum()
        self.leg_mass = self._calculate_leg_mass()
        ...
        feet_force_sensor_names = [
            "front_right_foot_to_floor_force_sensor",
            "front_left_foot_to_floor_force_sensor",
            "hind_right_foot_to_floor_force_sensor",
            "hind_left_foot_to_floor_force_sensor",
        ]
        self.feet_force_sensor = [
            self._mj_model.sensor(f'{foot_sensor_name}').id
            for foot_sensor_name in feet_force_sensor_names
        ]
        ...
        # Observation Size:
        self.num_observations = 37 + self.nu
    ...
    def _calculate_leg_mass(self) -> float:
        """Calculates 1/2 * m_leg from the MuJoCo model for the swing kinetic energy penalty."""
        # Using the front-left leg as the standard for all four legs
        leg_body_names = [
            'front_left_hip',
            'front_left_thigh',
            'front_left_calf'
        ...
        for name in leg_body_names:
            ...
            if body_id != -1:
                leg_mass += self._mj_model.body_mass[body_id]
