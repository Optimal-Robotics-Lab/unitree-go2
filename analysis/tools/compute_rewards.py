...
def compute_rewards(directory_name: str) -> None:
    ...
    state_accelerations = state_history[:, -12:]
    ...
    base_velocities = np.concatenate([global_velocity, local_angular_velocity], axis=1)
    ...
    # Running Gait Timers and Peak Timings:
    running_air_time = np.zeros(contact_forces.shape)
    running_contact_time = np.zeros(contact_forces.shape)

    # These will hold the snapshot of the last completed phase duration
    peak_air_time = np.zeros(contact_forces.shape)
    peak_contact_time = np.zeros(contact_forces.shape)

    for i, contact in enumerate(contact_forces):
        contact = contact.astype(bool)
        if i == 0:
            running_air_time[i] = np.where(contact, 0.0, dt)
            running_contact_time[i] = np.where(~contact, 0.0, dt)
        else:
            # Snapshot the timers right as the foot transitions state
            peak_air_time[i] = np.where(contact, running_air_time[i-1], peak_air_time[i-1])
            peak_contact_time[i] = np.where(~contact, running_contact_time[i-1], peak_contact_time[i-1])

            # Update the running timers
            running_air_time[i] = np.where(contact, 0.0, running_air_time[i-1] + dt)
            running_contact_time[i] = np.where(~contact, 0.0, running_contact_time[i-1] + dt)
    ...
    def calculate_reward(
        ...
        rewards = {
            'tracking_linear_velocity': (
                _reward_tracking_velocity(
                    reward_data['command'][:2],
                    reward_data['local_linear_velocity'][:2],
                    kernel_sigma=0.25,
                )
            ),
            'tracking_angular_velocity': (
                _reward_tracking_yaw_rate(
                    reward_data['command'][-1],
                    reward_data['local_angular_velocity'][-1],
                    kernel_sigma=0.25,
                )
            ),
            ...
            'acceleration': _cost_acceleration(reward_data['joint_accelerations']),
            ...
            # ),
            'air_time': _reward_air_time(
                reward_data['running_air_time'],
                reward_data['running_contact_time'],
                reward_data['command'],
                reward_data['global_velocity'],
                mode_time=0.2,
                command_threshold=0.0,
            ...
            'gait_variance': _cost_gait_variance(
                reward_data['peak_air_time'],
                reward_data['peak_contact_time'],
            ),
    ...
    for i in range(command.shape[0]):
        reward_data = {
            ...
            'action': actions[i],
            'joint_accelerations': state_accelerations[i],
            ...
            'running_air_time': running_air_time[i],
            'running_contact_time': running_contact_time[i],
            'peak_air_time': peak_air_time[i],
            'peak_contact_time': peak_contact_time[i],
        }
    ...
    mean_rewards['episodic_reward'] = float(np.sum(rewards))
...
def _cost_action_rate(
    ...
    return np.sum(np.square(action - previous_action))
