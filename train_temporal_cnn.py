...
from training.envs.unitree_go2_temporal_cnn import unitree_go2_joystick
from training.envs.unitree_go2_temporal_cnn import config
from training.envs.unitree_go2_temporal_cnn import randomize
...
def main(argv=None):
    ...
    for training_type in training_types:
        ...
        # Setup agent:
        mlp_policy_layer_size = [512, 256, 128,]
        mlp_value_layer_size = [512, 256, 128,]
        ...
        cnn_policy_kernel_init = [jax.nn.initializers.lecun_uniform()] + 2 * [jax.nn.initializers.delta_orthogonal(jnp.sqrt(2.0))]
        policy_kernel_init = [hidden_init] * len(mlp_policy_layer_size) + [output_init]
        value_kernel_init = [hidden_init] * len(mlp_value_layer_size) + [output_init]
        policy_input_normalization = statistics.RunningStatistics(
            reference_input=reference_observation["state"][0],
        )
        value_input_normalization = statistics.RunningStatistics(
            reference_input=reference_observation["privileged_state"],
        )
        model = agent.TemporalCNNAgent(
            ...
            # CNN Architecture:
            cnn_policy_features=(32, 32, 32),
            cnn_policy_kernel_sizes=(4, 3, 2), 
            cnn_policy_strides=(2, 1, 1),
            cnn_policy_paddings='VALID',
            cnn_policy_activation=activation_fn,
            cnn_policy_kernel_init=cnn_policy_kernel_init,
            # MLP Architecture:
            mlp_policy_layer_sizes=mlp_policy_layer_size,
            mlp_value_layer_sizes=mlp_value_layer_size,
            mlp_activation=activation_fn,
            mlp_policy_kernel_init=policy_kernel_init,
            mlp_value_kernel_init=value_kernel_init,
        ...
        agent_metadata = checkpoint_utilities.AgentMetadata(
            observation_size=env.observation_size,
            action_size=env.action_size,
            policy_layer_sizes=mlp_policy_layer_size,
            value_layer_sizes=mlp_value_layer_size,
            policy_input_normalization='statistics.RunningStatistics( \
